...
def run(args):
    ...
    feat_length = 8192
    centers = torch.nn.Parameter(torch.rand(args.n_classes, feat_length).to(device), requires_grad=True)
    optimizer = torch.optim.Adam([
        {'params': model.parameters()},
        {'params': centers}
    ], lr=args.lr)

    pm_path = 'logs/whitebox/resnet18_projection_matrix.npy'
    train_whitebox(model, optimizer, trainloader, b, centers, args, pm_path, feat_length)
    ...
    sd_path = 'logs/whitebox/marked/resnet18.pth'
...
def main():
    ...
    parser.add_argument('--epochs', default=10, type=int, help='embed_epoch')
    parser.add_argument('--scale', default=0.001, type=float, help='for loss1')
    parser.add_argument('--gamma', default=0.001, type=float, help='for loss2')
